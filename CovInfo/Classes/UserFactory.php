...
class UserFactory implements IFactory{
    ...
    public function build($user_id){
        ...
        $this->user = new User($row["account_id"],$row["password"],$row["email_address"],$row["first_name"],$row["middle_name"],$row["last_name"],$row["nic_number"],
            $row["birth_day"],$row["gender"],$row["name"],$row["prov_name"],$row["moh_name"],$row["address"],$row["phone_number"],$row["status_id"],$row["vaccine_status_id"],$row["blood_type_name"],$row["user_type_name"],$row["medical_remarks"],$row["user_id"],$row["account_type"]);
        return $this->user;
    }
}